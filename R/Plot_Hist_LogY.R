...
#' @export
plot_hist_logY <- function(df, column, factor_cols, bins = NULL, binwidth = NULL) {
  ...
  if (is.null(binwidth)) {
    if (length(unique(x)) <= 20) {
      ...
      binwidth <- 2 * IQR(x, na.rm = TRUE) / (length(x)^(1/3))
      if (is.na(binwidth) || binwidth == 0) binwidth <- diff(range(x, na.rm = TRUE)) / 30
    }
  }
